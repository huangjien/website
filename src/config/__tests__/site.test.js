...
describe('siteConfig', () => {
  ...
  it('should have specific navigation items', () => {
    ...
    const settingsItem = siteConfig.navItems.find(item => item.label === 'Settings')
    expect(settingsItem).toEqual({ label: 'Settings', href: '/settings' })
    
    const aboutItem = siteConfig.navItems.find(item => item.label === 'About')
  ...
  it('should have correct social links', () => {
    ...
    expect(siteConfig.links.sponsor).toBe('https://www.buymeacoffee.com/huangjien')
  })
