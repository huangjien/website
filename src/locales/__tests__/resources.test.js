...
// Mock JSON imports
jest.mock('../ar.json', () => ({ 'test.key': 'Arabic test' }), { virtual: true })
jest.mock('../de.json', () => ({ 'test.key': 'German test' }), { virtual: true })
jest.mock('../en.json', () => ({ 'test.key': 'English test' }), { virtual: true })
jest.mock('../es.json', () => ({ 'test.key': 'Spanish test' }), { virtual: true })
jest.mock('../fr.json', () => ({ 'test.key': 'French test' }), { virtual: true })
jest.mock('../ga.json', () => ({ 'test.key': 'Irish test' }), { virtual: true })
jest.mock('../it.json', () => ({ 'test.key': 'Italian test' }), { virtual: true })
jest.mock('../ja.json', () => ({ 'test.key': 'Japanese test' }), { virtual: true })
jest.mock('../ko.json', () => ({ 'test.key': 'Korean test' }), { virtual: true })
jest.mock('../ru.json', () => ({ 'test.key': 'Russian test' }), { virtual: true })
jest.mock('../zh_CN.json', () => ({ 'test.key': 'Chinese Simplified test' }), { virtual: true })
jest.mock('../zh_TW.json', () => ({ 'test.key': 'Chinese Traditional test' }), { virtual: true })
...
describe('resources configuration', () => {
  ...
  it('should have all expected language keys', () => {
    const expectedLanguages = [
      'en', 'ru', 'ar', 'de', 'it', 'ja', 'ko', 'ga', 'es', 'fr', 'zh_CN', 'zh_TW'
    ]
  ...
  it('should include Chinese simplified translations', () => {
    ...
    expect(resources.zh_CN.translation['test.key']).toBe('Chinese Simplified test')
  })
  ...
  it('should include Chinese traditional translations', () => {
    ...
    expect(resources.zh_TW.translation['test.key']).toBe('Chinese Traditional test')
  })
  ...
  it('should maintain immutable structure', () => {
    ...
    expect(Object.keys(resources)).toEqual(expect.arrayContaining(originalKeys))
  })
})