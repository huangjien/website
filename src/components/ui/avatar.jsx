...
export const Avatar = ({ className, ...props }) => {
  return (
    <AvatarPrimitive.Root
      className={cn("inline-flex h-8 w-8 rounded-full overflow-hidden", className)}
      {...props}
    />
  );
};
...
export default function AvatarDefault({ src, alt, className, fallback, children, ...props }) {
  return (
    <AvatarPrimitive.Root className={cn("inline-flex h-8 w-8 rounded-full overflow-hidden", className)} {...props}>
      {children ?? (
        <>
          {src ? <AvatarPrimitive.Image src={src} alt={alt} className="object-cover h-full w-full" /> : null}
